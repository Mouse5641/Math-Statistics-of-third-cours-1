...
def create_new_window_for_y(data):
    ...
    def classes():
        image = Image.open(create_histogram_for_x(x, int(entry.get())))
        photo = ImageTk.PhotoImage(image)
        label = tk.Label(new_window, image=photo)
        label.photo = photo
        label.place(x=0, y=20)

        image = Image.open(create_distribution_function(x, int(entry.get())))
        photo = ImageTk.PhotoImage(image)
        label = tk.Label(new_window, image=photo)
        label.photo = photo
    ...
    x = sort_and_remove_duplicates(array_data[:, 1])
    ...
    transform_menu.add_command(label="Логарифмування", command=lambda: logs_data(x))
    transform_menu.add_command(label="Стандартизація", command=lambda: standr_data(x))
    transform_menu.add_command(label="Вилучення аномальних значень", command=lambda: change_image(x))

    # Додавання зображення з гістограмою
    image = Image.open(create_histogram_for_x(x))
    photo = ImageTk.PhotoImage(image)
    label = tk.Label(new_window, image=photo)
    label.photo = photo
    label.place(x=0, y=20)
    ...
    text_widget.insert(tk.END, move_every_fourth_element_to_new_line(x))

    text_widget1 = tk.Text(new_window, height=8, width=80)
    text_widget1.place(x=660, y=690)
    text_widget1.insert(tk.END, derivation_values(x))
    ...
    image = Image.open(create_distribution_function(x))
    photo = ImageTk.PhotoImage(image)
    label = tk.Label(new_window, image=photo)
    label.photo = photo
    label.place(x=660, y=20)
